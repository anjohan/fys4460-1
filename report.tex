...
\]
The mean squared displacement is calculated by LAMMPs, while ``sufficiently long time'' is determined by looking at the error of a linear fit; the error is first calculated using all the obtained data for \(\langle r^2(t)\rangle\), then more and more of the first time values are ignored until the error has decreased significantly.
\tikzexternaldisable
\begin{figure}[htbp]
    \centering
    \begin{tikzpicture}
...
for file in files:
    searchres = re.findall(r"f/data/msd_(.+).dat",file)
    if len(searchres) != 0:
        T = searchres[0]
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Mean squared displacement as a function of time for a few different temperatures.}%
    \label{fig:msdtime}
\end{figure}
