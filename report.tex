...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Diffusion coefficient of silicon as a function of equilibrium temperature. The diffusivity looks to be asymptotically increasing with temperature.}\label{fig:siD}
\end{figure}
...
for i in range(0,len(Ts),max(1,len(Ts)//10)):
    ...
    print(r"\addlegendentry{\(T = \SI{%d}{\kelvin}\)};" % eqTs[i])
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Mean squared displacement for water as a function of time for a few different equilibrium temperatures. A higher temperature means a higher average kinetic energy, resulting in more motion. For the smallest temperatures, the diffusivity is approximately zero, as the system is in a solid state.}%
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Diffusion coefficient for water as a function of equilibrium temperature. The diffusivity looks to be asymptotically increasing with temperature.}\label{fig:h2oD}
\end{figure}
\begin{figure}[htbp]
    \centering
    \begin{tikzpicture}
...
for i in indices:
    ...
    print(r"\addlegendentry{\(T = \SI{%d}{\kelvin}\)};" % eqTs[i])
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Radial oxygen-oxygen distribution in water function for a few different temperatures.}%
    \label{fig:rdfh2o}
\end{figure}
