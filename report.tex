...
In this part of the project, the atoms are modelled as interacting through short-range conservative forces, which should give a behaviour reasonably closed to the target system of the van der Waal equation of state.

By introducing the density, \(\rho=N/V\), and dividing by \(N\), the equation of state can be rewritten as
\[
    \qty(P+a\rho^2)(\rho^{-1}- b) = \rho k_\mathrm{B}T
    \implies P = \frac{\rho k_\mathrm{B}T}{\rho^{-1}-b} - a\rho^2
    = \rho^2\qty(\frac{k_\mathrm{B}T}{1-\rho b} - a)\,.
\]
The parameters \(a\) and \(b\) can be determined through running simulations for many pairs of \(\qty(\rho,T)\).
A fitting function from scipy determines the coefficients which make the predicted equation of state fit as well as possible with the obtained data.
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Pressure as a function of temperature and density.}\label{fig:presstemp}
\end{figure}
